...
#### BAYESIAN INVERSION ----
# for (i in 1:(nrow(monitor_data)-1)) {
#   monitor_data$d18_sw[nrow(monitor_data)-i+1] = monitor_data$d18_sw[nrow(monitor_data)-i]
#   monitor_data$dD_sw[nrow(monitor_data)-i+1] = monitor_data$dD_sw[nrow(monitor_data)-i]
# }
...
data_obs = list(
  ...
  "temp_obs" = temp,
  "RH_obs" = RH
  # "temp_obs" = monitor_data$T_min,
  # "RH_obs" = monitor_data$RH_max / 100
)
...
p1 = ggplot(data) +
  ...
  geom_point(aes(x = RH_mean, y = DRH * 1e2, fill = T_mean, alpha = night_RH_eff), 
             shape = 21, size = 2) +
...
p2 = ggplot(data) +
  ...
  geom_errorbar(aes(x = RH_mean, y = DT, 
                    ymin = DT - post_night_temp_sd, ymax = DT + post_night_temp_sd), 
                linewidth = .2, width = 0, color = "grey80") +
  geom_point(aes(x = RH_mean, y = DT, fill = T_mean, alpha = night_temp_eff), 
             shape = 21, size = 2) +
  scale_alpha_manual(values = c("positive" = 1, "negative" = 0.5)) +
...
ggsave("figures/scenario3.jpg", width = 7.9, height = 6.6, dpi = 500)
...
p6 = ggplot(data) +
  geom_errorbar(aes(x = RH_mean, y = post_dew_influx_ratio,
                ...
                linewidth = .2, width = 0, color = "grey80") +
  geom_point(aes(x = RH_mean, y = post_dew_influx_ratio, fill = T_mean, alpha = dew_influx_ratio_eff),
             shape = 21, size = 2) +
  scale_alpha_manual(values = c("positive" = 1, "negative" = 0.5)) +
  scale_fill_distiller(palette = "RdBu") +
  guides(alpha = "none") +
...
ggplot(data) +
  ...
  geom_point(aes(x = post_d18_w, y = post_d2_w, fill = RH_mean, alpha = dew_influx_ratio_eff), shape = 22, size = 2) +
  scale_alpha_manual(values = c("positive" = 1, "negative" = 0.5)) +
  scale_fill_distiller(palette = "RdBu", direction = 1) +
...
#### compare dp posteriors with measurements
p1 = ggplot(data, aes(x = d18_p, y = post_d18_w)) +
  ...
  geom_abline(slope = 1, intercept = 0) +
  geom_point(aes(fill = RH_mean, alpha = d18_w_eff), shape = 21, size = 3) +
  annotate("text", x = -19, y = 3, label = "a",
           hjust = 0, size = 5, fontface = "bold") +
  scale_fill_distiller(palette = "RdBu", direction = 1) +
  scale_alpha_manual(values = c("positive" = 1, "negative" = .5)) +
  theme_bw() + theme +
  guides(alpha = "none") +
...
p2 = ggplot(data, aes(x = d2_p, y = post_d2_w)) +
  geom_errorbar(aes(ymin = post_d2_w - post_d2_w_sd,
                ...
                linewidth = .2, color = "grey80") +
  geom_abline(slope = 1, intercept = 0) +
  geom_point(aes(fill = RH_mean, alpha = d2_w_eff), shape = 21, size = 3) +
